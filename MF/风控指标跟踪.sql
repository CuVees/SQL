...
sum(case when dq_days>0 AND overdue>=1 then 1 else 0 end)/sum(case when dq_days>=1 then 1 else 0 end) as value,
'逾期一天' as metric
from 
(
SELECT 
...
select 
UNIX_TIMESTAMP(t.shouldback_time) as time_sec,
sum(case when dq_days>0 AND overdue>=3 then 1 else 0 end)/sum(case when dq_days>=3 then 1 else 0 end) as value,
'逾期三天' as metric
from 
(
SELECT 
...
select 
UNIX_TIMESTAMP(t.shouldback_time) as time_sec,
sum(case when dq_days>0 AND overdue>=7 then 1 else 0 end)/sum(case when dq_days>=7 then 1 else 0 end) as value,
'逾期七天' as metric
from 
(
SELECT 
